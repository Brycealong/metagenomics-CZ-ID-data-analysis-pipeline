...
PWY_combined_kegg_filtered$allTiers <- paste(PWY_combined_kegg_filtered$category, 
                                             ...
                                             sep = ".")
PWY_combined_kegg_filtered$allTiers <- gsub("[ -/:]", ".", PWY_combined_kegg_filtered$allTiers)
```
...
```{r}
PWY_counts_matrix <- as.matrix(t(PWY_sample_absolute_abundance_wider[,PWY_sig_up$feature]))
```

```{r}
metadata <- metadata[order(metadata$diseases_and_conditions),]
```



```{r fig.height=10, fig.width=20}
# Heatmap(PWY_counts_matrix, column_order = rownames(metadata))
# png("PWY_sig_up_heatmap.png", res=600, width=20, height=10, unit="in")
pdf("PWY_sig_up_heatmap.pdf", width=20, height=10)
pheatmap(PWY_counts_matrix[,rownames(metadata)], annotation_col = metadata %>% select(diseases_and_conditions), scale = "row", cluster_cols = FALSE, cluster_rows = FALSE)
dev.off()
# as.ggplot(pheatmap(PWY_counts_matrix[,rownames(metadata)], annotation_col = metadata %>% select(diseases_and_conditions), scale = "row", cluster_cols = FALSE, cluster_rows = FALSE))
# ggsave("PWY_sig_up_heatmap.png", height = 10, width = 20, units = "in", dpi = 600)
...
```{r}
####PWYs absolute abundance####

PWY_sample_absolute_abundance_wider <- PWY_combined_kegg_filtered %>%
  select(sample_name, allTiers, total) %>%
  pivot_wider(names_from = allTiers, values_from = total)

PWY_sample_absolute_abundance_wider <- PWY_sample_absolute_abundance_wider %>%
  column_to_rownames(var = "sample_name")
...
```{r}
fit_data <- Maaslin2(
  input_data = PWY_sample_absolute_abundance_wider, 
  input_metadata = metadata, 
  output = 'abx_vs_untreated_pwy_absolute_output',
  normalization = "TSS",
  transform = "NONE",
  min_prevalence = 0.05,
  max_significance = 0.25,
...
# volcano plot
#Wrangle the data
PWY_dexp <- read_tsv("abx_vs_untreated_pwy_absolute_output/all_results.tsv")

# Extract all significant results for ORA
PWY_sigs <- dplyr::filter(PWY_dexp, qval < 0.05)
# limiting results by log2FoldChange; may want to use log2FoldChange<0
PWY_sig_down<- dplyr::filter(PWY_dexp, qval < 0.05 & coef < 0)
# limiting results by log2FoldChange; may want to use log2FoldChange>0
PWY_sig_up<- dplyr::filter(PWY_dexp, qval < 0.05 & coef > 0)
...
ggsave("PWY_volcano_absolute_maaslin2.png", width = 10, height = 5, units = "in")
```
